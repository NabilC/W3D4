...
def harrison_ford
  ...
  Movie.joins(:actors).where.not(castings: {ord: 1}).where(actors: {name: 'Harrison Ford'}).select(:id, :title)
end
...
def biggest_cast
  ...
  Movie.joins(:castings).group(:id).select(:id, :title).order('COUNT(castings.id) DESC').limit(3)
end
...
def directed_by_one_of(them)
  ...
  Actor.select('movies.id, movies.title').joins(:directed_movies).where('actors.name IN (?)', them )
end
